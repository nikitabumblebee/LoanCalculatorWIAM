...
struct LoanModel: Hashable, Identifiable {
    var id: UUID = UUID()
    var amount: Double = UserDefaults.standard.lastAmount == 0.0 ? 10_000 : UserDefaults.standard.lastAmount
    var period: Int = UserDefaults.standard.lastPeriod == 0 ? 14 : UserDefaults.standard.lastPeriod
    ...
    var returnAmount: Double {
        ...
        let creditDuration = Double(period) / Double(days)
    ...
    var returnDate: Date {
        let calendar = Calendar.current
        return calendar.date(byAdding: .day, value: period, to: Date()) ?? Date()
    }
}
